...
## Test Génériques Remplacements
    ...
    function remplacer ($field, $arrayfrom, $arrayto, $valeur2, $DBchamp) {
        ...
        if ($newvaleur != $valeur2) {
            if (($this->idpeople != 'norecord') and ($this->idpeople >= 1)) {
    ...
    function isalpha ($field, $valeur) {
        $accepte = array("|'|", "|â|", "|à|", "|é|", "|è|", "|ê|", "|ë|", "|ç|", "|ö|", "|[A-Z]|", "|[a-z]|", "| |", "|-|", "|\.|");
...
###### Test des champs ###############################################################
    ...
    function checkCCC ($categorie) {
        ...
        if ($this->estvide ($nomchamp, $categorie) == 'OK') {
            if (!in_array($categorie, array('1', '3', 'E'))) {
                $erreur['Champ']  = $nomchamp;
                $erreur['Valeur'] = $categorie;
                $erreur['Infos']  = 'Contient une valeur non permise (Employé, Etudiant, Indépendant)';
                $this->ErrGS[]    = $erreur;
            }
        }
    }
    ...
    function checkNCF ($compte) {
        ...
        $compte = $this->remplacer($nomchamp, $carfrom, $carto, $compte, $DBnomchamp);
            ...
            if ((preg_match("/([0-9]{3})([0-9]{7})([0-9]{2})/", $compte, $regs)) and (strlen($compte) == 12)) {
                ...
                $erreur['Infos'] = 'Le numéro de compte doit tre sous le format XXXXXXXXXXXX';
                $this->ErrGS[] = $erreur;
            }
        }
    }

    function checkZ75 ($iban) {

        $nomchamp = 'Code IBAN';
        $DBnomchamp = 'iban';

        $iban = $this->triminvisibles ($nomchamp, $iban, $DBnomchamp);
        if ($this->estvide ($nomchamp, $iban) == 'OK') {

        $carfrom = array('-', '.', ' ');
        $carto = "";

        $iban = $this->remplacer($nomchamp, $carfrom, $carto, $iban, $DBnomchamp);

            if (preg_match("/([A-Z]{2})([0-9]{2})/", $iban, $regs)) {
#               $cleancompte = $regs[1].$regs[2].$regs[3];
#               $this->modulo97 ($nomchamp, $cleancompte);
            } else {
                $erreur['Champ'] = $nomchamp;
                $erreur['Valeur'] = $iban;
                $erreur['Infos'] = 'Le Numéro IBAN ne comporte pas de code PAYS';
                $this->ErrGS[] = $erreur;
            }
        }
    }
    ...
    function checkBIC ($bic) {
        ...
        $bic = $this->triminvisibles ($nomchamp, $bic, $DBnomchamp);
        if ($this->estvide ($nomchamp, $bic) == 'OK') {

        $carfrom = array('-', '.', ' ');
        $carto = "";

        $bic = $this->remplacer($nomchamp, $carfrom, $carto, $bic, $DBnomchamp);

#           if ((preg_match("/([0-9]{3})([0-9]{7})([0-9]{2})/", $bic, $regs)) and (strlen($bic) == 12)) {
#               $cleancompte = $regs[1].$regs[2].$regs[3];
#               $this->modulo97 ($nomchamp, $cleancompte);
#           } else {
#               $erreur['Champ'] = $nomchamp;
#               $erreur['Valeur'] = $bic;
#               $erreur['Infos'] = 'Le code BIC est invalide';
#               $this->ErrGS[] = $erreur;
#           }
        }
    }
...
#                       if (($naissance[2] == '01') and ($naissance[1] == '01')) {$naissance[1] = '00';} # Semble etre un cas particulier pour les pers nées le prémier janvier
                        if (($regs[3] == $naissance[2]) and (($regs[2] == $naissance[1]) or ($regs[2] == ($naissance[1] + 40))) and ($regs[1] == substr($naissance[0], -2))) {
    ...
    function checkRPE ($registre) {
        ...
        if ($registre < 1) {
        # Récupère le prochain num de registre
            $nreg = new db();
            $nextreg = $nreg->CONFIG('nextregperso');
        # met a jour le num de registre dans people
            $idp = $this->idpeople;
            $people = new db();
            $people->inline("UPDATE `people` SET `$DBnomchamp` = '$nextreg' WHERE `idpeople` ='$idp';");
        # Incrémente le num de registre suivant
            $nextreg++;
            $updreg = new db();
    ...
    function checkNUB ($nbte) {

        $nomchamp = 'Numéro de boite';
        $DBnomchamp = 'bte1';

        $nbte = $this->triminvisibles ($nomchamp, $nbte, $DBnomchamp);

        if (!empty($nbte)) {
            $this->isalnum ($nomchamp, $nbte) ;
        }
    }
    ...
    function checkZ05 ($datein) {
        ...
        # Recherche de la date de la première mission du people
            $animdate[] = $DB->getOne("SELECT MIN(datem) FROM `animation` WHERE `idpeople` = '$idp' AND datem > '2003-01-01'");
            $animdate[] = $DB->getOne("SELECT MIN(vipdate) FROM `vipmission` WHERE `idpeople` = '$idp' AND vipdate > '2003-01-01'");
            $animdate[] = $DB->getOne("SELECT MIN(datem) FROM `merch` WHERE `idpeople` = '$idp' AND datem > '2003-01-01'");
            sort ($animdate);

            foreach ($animdate as $ladate) {
                if (($ladate != '0000-00-00') and (!empty($ladate))) $gooddate[] = $ladate;
            }

            $zegooddate = $gooddate[0];

            $dateentr = $DB->getOne("SELECT `$DBnomchamp` FROM `people` WHERE `idpeople` ='$idp';");

            if ($dateentr != $zegooddate) {

                # Modification de la date d'entrée en service
                $DB->inline("UPDATE `people` SET `$DBnomchamp` = '$zegooddate' WHERE `idpeople` ='$idp';");

                $erreur['Champ'] = $nomchamp;
                $erreur['Valeur'] = $datein;
                $erreur['ValeurCorrige'] = $zegooddate;

                if (($dateentr[$DBnomchamp] == '0000-00-00') or ($dateentr[$DBnomchamp] == '')) {
                    $erreur['Infos'] = 'Date d\'entrée ajoutée';
                } else {
                    $erreur['Infos'] = 'Modification de la date d\'entree';
                }
