...
$tempFolder = "/NRO/tmp";       //Make sure that this is the same as in upload.cgi.
$moveToFolder = "/NRO/media/photoweb"; //Where the files will be moved upon upload, leave blank if you want it to be the same as $tempFolder.
...
$idwebpeople = $request->idwebpeople;

if (substr($sessionID, -1, 1) == '1') $sfx = '-b';
else $sfx = '';
...
if ($percent_done >= 100) {
  ...
  $fileName = trim(stripslashes(urldecode($fileName)));
        $handle2 = fopen($moveToFolder. '/' .$idwebpeople.$sfx.'.jpg', 'wb');
	$file = array();
