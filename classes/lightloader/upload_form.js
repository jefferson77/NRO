...
function uploadForm(form, sid) {
  ...
  if (!allowSubmit) {
    alert("Le type de fichier "+ext+" n'est pas supporté. Les formats acceptés sont : .jpg, .jpeg, .gif, .png et .bmp.\n\nMerci !");
  return;
  }
  ...
  $no = sid.charAt((sid.length)-1);
  if ($no == "0")
     request.idwebpeople = document.forms["form0"].elements["idpeople"].value;
  else
     request.idwebpeople = document.forms["form1"].elements["idpeople"].value;
  ...
  repeater = new RepeatGetAction('/classes/lightloader/filestatus.php', request, ul_vars.interval);
  repeater.successFunc = function (getBack) {
    if (getBack.progress >= 100 || getBack.progress=='done') {
      
      $no = getBack.sid.charAt((getBack.sid.length)-1);
      if ($no == "0")
          $idpeople = document.forms["form0"].elements["idpeople"].value;
       else
          $idpeople = document.forms["form1"].elements["idpeople"].value;
          
      getBack.progress = 100;

      if ($no=='0')
        document.getElementById('img'+$no).innerHTML = '<div align="center"><img src="/data/people/photo.php?id='+$idpeople+'&rep=web&noscissors=true"></div>';
      else
		document.getElementById('img'+$no).innerHTML = '<div align="center"><img src="/data/people/photo.php?id='+$idpeople+'&sfx=-b&rep=web&noscissors=true"></div>';
		this.stop();
    }
