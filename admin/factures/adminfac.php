...
if (!isset($_POST['dact'])) $_POST['dact'] = '';
...
switch ($_GET['act']) {
    ...
    ###########################################################################################
        ...
        switch (@$_POST['dact']) {
            ...
            case 'S':
                if ($_POST['idman'] > 0) $DB->inline("DELETE FROM facmanuel WHERE idman = ".$_POST['idman']);
                break;
...
#/############# SWITCH des PURGE FL MERCH #############################################
    ...
    default:
        ...
        if (!empty($_POST['search'])) {
            ...
            ## secteur
            if(isset($_POST['secteur']) && is_array($_POST['secteur'])) $quid .= " AND f.secteur in ('".implode("', '", $_POST['secteur'])."')";

            ## modefac
            if(isset($_POST['modefac']) && is_array($_POST['modefac'])) $quid .= " AND f.modefac in ('".implode("', '", $_POST['modefac'])."')";

            ## echéance
            if (isset($_POST['echeance']) && is_array($_POST['echeance'])) {
