...
if (!empty($idvipjob)) {
		$stocksort='stm.idstockf, stm.reference, ma.idstockm, ma.idvip, ma.mnom';
		$recherche1='
			SELECT 
		...
		$quid = " WHERE m.idvipjob = ".$idvipjob." AND ma.dateout >= '".date ("Y-m-d", strtotime("-2 months"))."'";
...
$j++; # compteur modele
?>
	<?php /* Ligne Famille */ if ($row['idstockf'] != $idstockf) { $idstockf = $row['idstockf'];?>
		<tr>
