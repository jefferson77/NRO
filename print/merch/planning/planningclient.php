<?php
#########################################################################################################################################################################
### Page de Planning Client en MERCH ######################################################################################################################################
#########################################################################################################################################################################
...
function path_planningclientmerch($week, $year, $idcofficer, $genre) {

## declarations
    $fname = $year.'-'.$week;

    $lespath['dossier'] = Conf::read('Env.root').'document/merch/'.cleanalpha($genre).'/planningclient/'.prezero($idcofficer, 5).'/';
    $lespath['filename'] = $fname.'.pdf';
    $lespath['full'] = $lespath['dossier'].$lespath['filename'];
    $lespath['history'] = Conf::read('Env.root').'history/merch/'.cleanalpha($genre).'/planningclient/'.prezero($idcofficer, 5).'/';
...
function print_planningclientmerch($week, $year, $idcofficer, $genre)
{
###### GEt infos ######
	global $DB;
    $DB->inline("SET NAMES latin1");

	$infosofficer = $DB->getRow("SELECT
		co.langue, c.societe, co.fax,
		co.onom, co.oprenom
		FROM cofficer co
			LEFT JOIN client c ON co.idclient = c.idclient
		WHERE co.idcofficer = ".$idcofficer);
	...
	if (count($listemission) > 0)
	{
		...
		$path = path_planningclientmerch($week, $year, $idcofficer, $genre);
		...
		pdf_set_info($pdf, "Title", "Planning Merch Client");
		...
		switch ($infosofficer['langue'])
		{
			...
			default:
				$phrase = array('');
				echo '<br> Langue pas d&eacute;finie pour l\'officer : '.$infosofficer['onom'].' '.$infosofficer['oprenom'].' ('.$infosofficer['idcofficer'].')';
		}
		...
		foreach ($listemission as $row) {
			if ($tour == 1)
			{
				...
				pdf_show_boxed($pdf, $phrase[13] , 150 , 500 , 200, 32 , 'left', "");
				...
				pdf_show_boxed($pdf, $phrase[9]." : " , 350 , 520 , 100, 20 , 'right', ""); 		# Titre
				...
				pdf_show_boxed($pdf, $infosofficer['societe'] , 450 , 520 , 525, 20 , 'left', ""); 		# Titre
				pdf_show_boxed($pdf, $infosofficer['onom']." ".$infosofficer['oprenom'] , 450 , 495 , 525, 20 , 'left', ""); 		# Titre
				pdf_show_boxed($pdf, $infosofficer['fax'] , 450 , 470 , 525, 20 , 'left', ""); 		# Titre
			...
			foreach ($dim as $key => $val) {
				pdf_show_boxed($pdf, utf8_decode($datas[$key]) , $x + 2 , $tabt , $val['l'] - 4, $ht, $val['a'], "");
				$x += $val['l'];
			}
