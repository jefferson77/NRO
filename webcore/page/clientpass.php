...
#		$detail = new db();
#		$detail->inline("SELECT * FROM `supplier` WHERE `c0` = '$c0'");
...
<table border="0" cellpadding="0" cellspacing="5" width="80%" align="center">
	...
	<tr>
		...
		<!-- -=-=-=-=-=-=-=-=-=-=-=-=- Fin new  -=-=-=-=-=-=-=-=-=-=-=-=-=-=- -->
		<FIELDSET>
			<legend>Client Secured Area</legend>
			<!-- -=-=-=-=-=-=-=-=-=-=-=-=- Debut log -=-=-=-=-=-=-=-=-=-=-=-=-=-=- --> 
		...
		<table border="0" cellpadding="0" cellspacing="5" width="100%">
			...
			#### input
				...
				<form action="<?php echo $PHP_SELF; ?>" method="post" name="CelsysForm" onsubmit="return scanForm(this)">
							Please send me my password at :<br>
...
Thank you for your co-operation.
							...
							# fin du corps du message
								...
								$mail->SMTPAuth = false;
								$mail->From = "info@virtual-words.com";
								$mail->FromName = "info@virtual-words.com";
								$mail->AddAddress($c0, $c0);
								$mail->AddBCC('database@virtual-words.com', 'virtual-words');
								$mail->WordWrap = 80;
								$mail->IsHTML(false);
